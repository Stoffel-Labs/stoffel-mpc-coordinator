...
pub mod on_chain {
    ...
    pub async fn ws_connect(addr: &str, wallet_sk: &str) -> impl Provider + Clone {
    ...
    pub async fn setup_coord(eth: impl Provider + Clone, contract_addr: Address) -> OnChainCoordinator<impl Provider + Clone> {
        let coord_instance = FakeCoordinator::new(contract_addr, eth.clone());
        OnChainCoordinator::new(coord_instance).await
    }
    
    impl<P: Provider + Clone> OnChainCoordinator<P> {
        pub async fn new(coord: FakeCoordinatorInstance<P>) -> Self {
            let contract_block = Self::coord_creation_block(&coord).await;
            Self { coord, contract_block }
        }
    ...
    mod tests {
        ...
        pub async fn sig_gen_onchain() {
            ...
            let mut coord = OnChainCoordinator::new(contract.clone()).await;
            ...
            // simulate 2 * t + 1 = 3 nodes that have received valid signatures from a client
            {
                ...
                let coord = OnChainCoordinator::new(instance).await;
        ...
        pub async fn coord_creation_block() {
            ...
            let coord = OnChainCoordinator::new(coord_instance).await;
        ...
        pub async fn end_to_end() {
            ...
            tokio::spawn({
                ...
                // grant roles to parties
                let mut coord = OnChainCoordinator::new(instances[0].clone()).await;
                coord.grant_roles(initial_mpc_nodes.clone()).await.expect("granting roles failed");
            ...
            tokio::spawn({
                ...
                let instance = FakeCoordinatorInstance::new(*contract.address(), provider.clone());
                let mut coord = OnChainCoordinator::new(instance).await;
...
pub mod off_chain {
    ...
    impl OffChainCoordinator {
        pub async fn start_coord_from_cert(addr: &str, port: u16, prog_hash: [u8; 32], n: u64, t: u64, initial_mpc_nodes: Vec<ClientIdentity>, n_inputs: u64, cert: Arc<rcgen::CertifiedKey<rcgen::KeyPair>>) -> Self {
        ...
        pub async fn start_coord(addr: &str, port: u16, prog_hash: [u8; 32], n: u64, t: u64, initial_mpc_nodes: Vec<ClientIdentity>, n_inputs: u64, cert_der: Vec<u8>, key_der: Vec<u8>) -> Self {
    ...
    mod tests {
        ...
        async fn start_client_server() {
            ...
            let _ = OffChainCoordinator::start_rpc_client_from_cert(addr, port, timestamp, client_cert()).await;
        }
        ...
        async fn trigger_pp() {
            ...
            // event triggered BEFORE waiting for the event
            {
                ...
                let node0 = OffChainCoordinator::start_rpc_client_from_cert(addr, port, timestamp, certs.remove(0)).await;
                let node1 = OffChainCoordinator::start_rpc_client_from_cert(addr, port, timestamp, certs.remove(0)).await;
