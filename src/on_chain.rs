...
use alloy_primitives::{Address, Bytes, FixedBytes, Keccak256, Signature, U256};
...
pub mod node_rpc {
    ...
    use crate::{rpc::ClientInfo, CoordinatorError};
    use ark_serialize::CanonicalSerialize;
    ...
    impl<P: Provider + WalletProvider + Clone + 'static> NodeRPCServer<P> {
        ...
        pub async fn start(
            ...
            let server_handle = crate::rpc::start_coord::<NodeRPCServerConnection<P>>(
                addr,
                port,
                cert_der,
                key_der,
    ...
    impl<P: Provider + WalletProvider + Clone + 'static> crate::rpc::RPCServerConnection
        for NodeRPCServerConnection<P>
    {
    ...
    #[async_trait]
    impl<P: Provider + WalletProvider + Clone + 'static> OnChainNodeRPCServer
        for NodeRPCServerConnection<P>
    {
    ...
    pub async fn grant_roles(&self, nodes: Vec<Address>) -> Result<(), CoordinatorError> {
        ...
        for i in 0..nodes.len() {
            ...
            match builder.send().await {
                Ok(r) => match r.watch().await {
                    ...
                    Err(e) => {
                        return Err(CoordinatorError::EthereumError(format!(
                            "error waiting for transaction to grant role to be mined: {}",
                            e
                        )));
                    }
                },
                Err(e) => {
                    return Err(CoordinatorError::EthereumError(format!(
                        "error sending transaction to grant role: {}",
                        e
                    )));
                }
            }
...
impl<P: Provider + WalletProvider + Clone> Coordinator<Fr> for OnChainCoordinator<P> {
    ...
    async fn wait_for_indices(
        ...
        ))) = events.next().await
        {
            addr_to_i.insert(
                client,
                vec![u256_to_u64(reservedIndex).expect("conversion from U256 to u64 failed")],
            );
            eprintln!(
    ...
    ) -> Result<HashMap<ClientIdentity, Vec<RobustShare<Fr>>>, CoordinatorError> {
        ...
        for _ in 0..n_clients {
            match events.next().await {
                Some(Ok((
                    ...
                    let masked_input =
                        u256_to_fr(maskedInput).ok_or(CoordinatorError::U256ToFrError)?;
    ...
    async fn send_masked_input(&self, masked_input: Fr, i: u64) -> Result<(), CoordinatorError> {
        let builder = self
            .coord
            .submitMaskedInput(fr_to_u256(masked_input), U256::from(i));
        match builder.send().await {
            Ok(r) => match r.watch().await {
                Ok(_) => Ok(()),
                Err(e) => Err(CoordinatorError::EthereumError(format!(
    ...
    async fn obtain_outputs(&self) -> Result<Vec<Fr>, CoordinatorError> {
        ...
        while let Some(Ok((
            StoffelCoordinator::EnoughPrivateOutputShares { client: _, shares },
            _,
        ))) = events.next().await
        {
        ...
        Err(CoordinatorError::EthereumError(
            "event stream ended unexpectedly while waiting for EnoughPrivateOutputShares events"
                .to_string(),
        ))
    }
    ...
    async fn send_output_shares(
        ...
        let builder = self
            .coord
            .sendPrivateOutputShares(client_id, Bytes::from(bytes));
        let result = builder.send().await;
...
mod tests {
    ...
    pub async fn start_node_rpc() {
        ...
        let _ = super::node_rpc::NodeRPCClient::start_rpc_client_from_cert(
            t,
    ...
    pub async fn end_to_end() {
        ...
        coords[0]
            .grant_roles(initial_mpc_nodes.clone())
            .await
            .expect("granting roles failed");
    ...
    pub async fn reset_and_rerun() {
        ...
        coords[0]
            .reset_coord(hash.0, t, initial_mpc_nodes.clone(), 1)
            .await
            .unwrap();
