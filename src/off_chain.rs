use crate::{
    round_before,
    rpc::{ClientInfo, FieldElement},
    Coordinator, CoordinatorError, Round,
};
...
pub mod node_rpc {
    ...
    use crate::{rpc::ClientInfo, CoordinatorError, NodeRPCError};
    use ark_ff::FftField;
    ...
    impl<F: FftField> NodeRPCServer<F> {
        ...
        pub async fn start(addr: &str, port: u16, cert_der: Vec<u8>, key_der: Vec<u8>) -> Self {
            ...
            let server_handle = crate::rpc::start_coord::<NodeRPCServerImpl<F>>(
                addr,
                port,
                cert_der,
                key_der,
...
impl<F: FftField> CoordinatorRPCBaseServer<F> for CoordinatorRPCServerConnectionBase<F> {
    ...
    async fn reserve_mask_index(&self, i: u64) -> RpcResult<()> {
        ...
        let event = Event::<F>::ReservedInputEvent {
            client: self.id.clone(),
            reserved_index: i,
        };
...
impl<F: FftField> OffChainCoordinatorClient<F> {
    ...
    pub async fn trigger_round(&self, round: Round) -> Result<(), CoordinatorError> {
        CoordinatorRPCBaseClient::<F>::transition(self.rpc(), round)
            .await
...
impl<F: FftField> Coordinator<F> for OffChainCoordinatorClient<F> {
    ...
    async fn reset_coord(
        ...
        CoordinatorRPCBaseClient::<F>::reset(
            self.rpc(),
            prog_hash,
            3 * t + 1,
            t,
    ...
    async fn wait_for_indices(
        ...
        let mut sub =
            CoordinatorRPCBaseClient::<F>::sub_reserved_indices(self.rpc(), self.get_timestamp())
                .await
                .unwrap();
    ...
    ) -> Result<HashMap<ClientIdentity, Vec<RobustShare<F>>>, CoordinatorError> {
        // Wait for masked input events.
        let mut sub =
            CoordinatorRPCBaseClient::<F>::sub_masked_inputs(self.rpc(), self.get_timestamp())
                .await
                .map_err(|e| CoordinatorError::JSONError(e.to_string()))?;
    ...
    async fn wait_for_round(&self, round: Round) -> Result<(), CoordinatorError> {
        let mut sub =
            CoordinatorRPCBaseClient::<F>::sub_round(self.rpc(), round, self.get_timestamp())
                .await
                .map_err(|e| CoordinatorError::JSONError(e.to_string()))?;
    ...
    async fn send_masked_input(&self, masked_input: F, i: u64) -> Result<(), CoordinatorError> {
        match CoordinatorRPCBaseClient::<F>::submit_masked_input(
            self.rpc(),
            FieldElement {
                value: masked_input,
            },
    ...
    async fn reserve_mask_index(&mut self, i: u64) -> Result<(), CoordinatorError> {
        CoordinatorRPCBaseClient::<F>::reserve_mask_index(self.rpc(), i)
            .await
            .map_err(|e| CoordinatorError::JSONError(e.to_string()))
    }
        ...
        while let Some(Ok(enc_output_shares)) = sub.next().await {
            ...
            let outputs: Vec<_> = (0..self.n_outputs.unwrap() as usize)
                .filter_map(|i| {
                    ...
                    match RobustShare::recover_secret(
                        &shares_i,
                        (4 * self.t + 1) as usize,
                        self.t as usize,
                    ) {
    ...
    async fn send_output_shares(
        ...
        if let Err(e) =
            CoordinatorRPCBaseClient::<F>::send_output_shares(self.rpc(), client_id, c).await
        {
            return Err(CoordinatorError::JSONError(e.to_string()));
        }
...
mod tests {
    ...
    async fn start_client_server() {
        ...
        let t = 1;
        let server_state =
            CoordinatorRPCServerSharedBase::<Fr>::new([0u8; 32], 5, t, public_keys, 1);
        ...
        let _ = OffChainCoordinatorClient::<Fr>::start_rpc_client_from_cert(
            addr,
            port,
            timestamp,
            1,
    ...
    async fn trigger_pp() {
        ...
        // event triggered BEFORE waiting for the event
        {
            ...
            let node0 = OffChainCoordinatorClient::<Fr>::start_rpc_client_from_cert(
                addr,
                port,
                timestamp,
                1,
            ...
            .await;
            let node1 = OffChainCoordinatorClient::<Fr>::start_rpc_client_from_cert(
                addr,
                port,
                timestamp,
                1,
    ...
    async fn end_to_end() {
        ...
        let server_state =
            CoordinatorRPCServerSharedBase::<Fr>::new([0u8; 32], n, t, public_keys.clone(), 1);
        let coord = OffChainCoordinatorServer::<FakeCoordinatorConnection>::start_coord_from_cert(
            server_state,
            coord_addr,
            coord_port,
        ...
        tokio::spawn({
            ...
            for i in 0..3 {
                let coord = OffChainCoordinatorClient::<Fr>::start_rpc_client_from_cert(
                    coord_addr,
                    coord_port,
                    timestamp,
                    1,
            ...
            let mask_shares =
                RobustShare::compute_shares(correct_mask, n as usize, t as usize, None, &mut rng)
                    .unwrap();
            let output_shares =
                RobustShare::compute_shares(correct_output, n as usize, t as usize, None, &mut rng)
                    .unwrap();
        ...
        tokio::spawn({
            ...
            let mut coord = OffChainCoordinatorClient::<Fr>::start_rpc_client_from_cert(
                coord_addr,
                coord_port,
                timestamp,
                1,
            ...
            .await;
            let rpc_client: super::node_rpc::NodeRPCClient<Fr> =
                super::node_rpc::NodeRPCClient::start_rpc_client_from_cert(
                    t as usize,
    ...
    async fn end_to_end_fake_coord() {
        ...
        let t = 1;
        let server_state =
            CoordinatorRPCServerSharedBase::<Fr>::new([0u8; 32], 5, t, public_keys.clone(), 1);
        let coord = OffChainCoordinatorServer::<FakeCoordinatorConnection>::start_coord_from_cert(
            server_state,
            coord_addr,
            coord_port,
        ...
        tokio::spawn({
            ...
            let mut rng = test_rng();
            let mask_shares =
                RobustShare::compute_shares(correct_mask, n, t as usize, None, &mut rng).unwrap();
            let output_shares =
                RobustShare::compute_shares(correct_output, n, t as usize, None, &mut rng).unwrap();
        ...
        tokio::spawn({
            ...
            .await;
            let rpc_client = super::node_rpc::NodeRPCClient::<Fr>::start_rpc_client_from_cert(
                t as usize,
                node_rpc_addrs.clone(),
                cert.clone(),
            )
